...
export async function GET() {
  try {
    ...
    const [courtsResult, blockedResult] = await Promise.all([
      supabase.from("courts").select("id,name,is_active").order("id"),
...
export async function PATCH(request: Request) {
  try {
    const payload = (await request.json()) as { id?: number; is_active?: boolean };
    ...
    if (payload.is_active === undefined) {
      return NextResponse.json({ message: "is_active field is required." }, { status: 400 });
    }
    ...
    if (error) {
      return NextResponse.json(
        { message: "Failed to update court status.", details: error.message },
        { status: 500 },
      );
    }
