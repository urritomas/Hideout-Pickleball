      ...
      <section className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        {overviewCards.map((card) => (
          <article key={card.label} className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm">
            ...
            <p className="mt-1 sm:mt-2 font-display text-xl sm:text-2xl font-semibold text-slate-900">{card.value}</p>
            <p className="mt-1 sm:mt-2 text-xs font-medium text-emerald-600">{card.delta} this week</p>
          </article>
        ))}
      </section>
